...
export default function WorkflowSteps() {
  ...
  return (
    <div className="relative z-10 py-20 sm:py-24 px-4 sm:px-6 overflow-hidden w-full bg-transparent">
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:hidden gap-4">
          {steps.map((step, index) => (
            <div
              key={step.number}
              className="bg-white/5 border border-white/10 rounded-cards p-5 overflow-hidden h-full"
            >
              <div className="relative z-10">
                {/* Icon */}
                <div className="w-12 h-12 rounded-cards bg-brand flex items-center justify-center text-black mb-4">
                  {step.icon}
                </div>

                {/* Content */}
                <h3 className="text-lg font-bold text-white mb-2 font-geist">{step.title}</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {step.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 rounded-full bg-white/5 border border-white/15 text-white/60 text-xs font-geist">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <div className="relative hidden md:block">
          <div
            ...
            className="max-h-[500px] sm:max-h-[600px] overflow-y-auto scrollbar-hide border border-white/10 rounded-cards w-full bg-black/30 backdrop-blur-md"
          >
            {/* Timeline line */}
            <div ref={timelineRef} className="relative px-4 sm:px-6 pt-20 sm:pt-24 md:pt-28 pb-20 sm:pb-24 md:pb-28 w-full">
              {/* Animated progress line */}
              <div
                className="absolute left-8 md:left-1/2 top-2 w-[2px] bg-brand will-change-height"
              ...
              {steps.map((step, index) => (
                ...
                >
                  ...
                  <div className={`absolute left-8 md:left-1/2 -translate-x-1/2 w-4 h-4 rounded-full border-4 z-10 transition-all duration-300 ${
                    activeStep === index ? 'bg-brand border-bg-white/5 scale-125' : 'bg-white/5 border-actionBlack'
                  }`} />

                  {/* Card */}
                  <div className={`w-full md:w-[38%] pl-12 md:pl-0 ${index % 2 === 0 ? 'md:pr-12' : 'md:pl-12'} ml-16 md:ml-0`}>
                    <div className="bg-white/5 border border-white/10 rounded-cards p-4 sm:p-6 hover:shadow-sm transition duration-300 overflow-hidden h-full min-h-[220px] sm:min-h-[240px] md:min-h-[260px]">
                      <div className="relative z-10">
                        {/* Icon */}
                        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-cards bg-brand flex items-center justify-center text-black mb-4 sm:mb-6">
                          {step.icon}
                        </div>

                        {/* Content */}
                        <h3 className="text-lg sm:text-xl font-bold text-white mb-2 font-geist break-words">{step.title}</h3>
                        <p className="text-white/60 text-xs sm:text-sm mb-4">{step.subtitle}</p>
                        <p className="text-white/60 text-sm leading-relaxed mb-4 sm:mb-6 break-words">
                          {step.description}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {step.tags.map((tag) => (
                            <span key={tag} className="px-2 sm:px-3 py-1 rounded-full bg-white/5 border border-white/15 text-white/60 text-xs font-geist">
                              {tag}
                            </span>
                          ))}
                        </div>
