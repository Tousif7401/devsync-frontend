...
const AnalyticsDemo = () => {
  ...
  const metrics = [
    { label: 'Engagement', value: '4.2%', color: '#ff9900', trend: '+23%' },
    { label: 'Reach', value: '12.4K', color: '#e68a00', trend: '+127%' },
    { label: 'Impressions', value: '48.2K', color: '#cc7a00', trend: '+89%' }
  ];
  ...
  return (
    <div className="bg-offWhiteSage rounded-cards border border-fadedStone p-6 h-[360px] flex items-center justify-center relative overflow-hidden">
      <div className="relative z-10 w-full">
        ...
        <div className="flex justify-center mb-6">
          <div className="relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              >
                <div className="text-3xl font-bold text-midnightInk">{metrics[activeMetric].value}</div>
                <div className="text-xs text-gunmetalGray mt-1">{metrics[activeMetric].label}</div>
              </motion.div>
            </div>
          </div>
        </div>
          ...
          {metrics.map((metric, i) => (
            <motion.button
              ...
              animate={{
                ...
                backgroundColor: i === activeMetric ? metric.color : '#ccc'
              }}
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-navigation bg-canvasWhite border border-fadedStone">
            <span className="text-emerald-600 text-sm font-medium">↑ {metrics[activeMetric].trend}</span>
            <span className="text-gunmetalGray text-xs">this week</span>
          </div>
        </motion.div>
        ...
        <div className="flex justify-end items-end gap-0.5 mt-4 h-8">
          {Array.from({ length: 24 }).map((_, i) => (
            <motion.div
              ...
              className="w-1 bg-highlightOrange/30 rounded-t"
            />
          ))}
        </div>
      </div>
    ...
    <div className="bg-offWhiteSage rounded-cards border border-fadedStone p-6 h-[360px] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 12 }).map((_, i) => (
          <motion.div
            ...
            className="absolute w-1 h-1 rounded-full bg-highlightOrange/20"
            style={{
              left: '50%',
              top: '50%'
            }}
      ...
      <div className="relative z-10">
        ...
        <motion.div
          ...
          className="w-32 h-32 mb-8 bg-gradient-to-br from-highlightOrange/20 to-highlightOrange/5 border border-highlightOrange/30 flex items-center justify-center"
        >
          <motion.span
            key={phase}
        ...
        <motion.div
          ...
          className="text-center"
        >
          <div className="text-midnightInk font-bold text-lg">{phases[phase].label}</div>
          <div className="text-highlightOrange text-sm">{phases[phase].sublabel}</div>
        </motion.div>
        ...
        <div className="flex justify-center gap-2 mt-6">
          {phases.map((_, i) => (
            <motion.div
              ...
              animate={{
                scale: i === phase ? 1.3 : 1,
                backgroundColor: i === phase ? '#ff9900' : '#ccc'
              }}
              transition={{ duration: 0.3 }}
      ...
      <div className="flex items-start gap-6 w-full max-w-lg relative">
        ...
        <div className="flex-1 relative" id="repo-section">
          <div className="text-center mb-4">
            <div className="text-xs text-gunmetalGray uppercase tracking-wide mb-2">Your Repository</div>
            <div className="w-16 h-16 mx-auto rounded-medium bg-canvasWhite border border-fadedStone flex items-center justify-center relative z-10" id="repo-icon">
          ...
          <div className="space-y-2">
            {dataItems.map((item, i) => (
              <motion.div
                ...
                className={`px-3 py-2 rounded-small text-xs font-mono ${
                  item.blocked
        ...
        <div className="relative flex items-center mt-8">
          <motion.div
            ...
            className="w-20 h-20 rounded-medium bg-canvasWhite border-2 border-emerald-300 flex items-center justify-center relative overflow-hidden"
          ...
          >
            ...
            <motion.div
              ...
              className="absolute left-0 right-0 h-8 bg-gradient-to-b from-transparent via-emerald-300/20 to-transparent"
            />
          </motion.div>
        </div>
        ...
        <div className="flex-1 relative" id="devsync-section">
          <div className="text-center mb-4">
            <div className="text-xs text-gunmetalGray uppercase tracking-wide mb-2">DevSync AI</div>
            <div className="w-16 h-16 mx-auto rounded-medium bg-canvasWhite border border-fadedStone flex items-center justify-center relative z-10 p-2" id="devsync-icon">
          ...
          <div className="space-y-2">
            {dataItems.filter(d => !d.blocked).map((item, i) => (
              <motion.div
                ...
                className="px-3 py-2 rounded-small text-xs font-mono bg-highlightOrange/10 text-highlightOrange border border-highlightOrange/20"
              >
...
const MultiPlatformDemo = () => {
  ...
  const getColorWithOpacity = (color: string, opacity: number) => {
    if (color === '#111111') return `rgba(17, 17, 17, ${opacity / 100})`;
    if (color.startsWith('#')) {
  ...
  return (
    <div className="bg-offWhiteSage rounded-cards border border-fadedStone p-6 h-[360px] flex flex-col items-center relative overflow-hidden">
      {/* Central card transformation */}
      <div className="relative w-72 h-48" style={{ perspective: '1000px' }}>
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="absolute inset-0 rounded-cards border-2 p-5 flex flex-col items-center justify-center bg-canvasWhite"
            style={{
          ...
          >
            ...
            <motion.div
              ...
              style={{ color: stages[stage].color === '#111111' ? '#111111' : stages[stage].color }}
            >
              {stages[stage].icon}
            </motion.div>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className="text-midnightInk font-semibold mb-2 text-sm"
            >
            ...
            <motion.div
              ...
              className="text-xs text-gunmetalGray text-center leading-relaxed px-2 line-clamp-2"
            >
      ...
      <div className="flex gap-2 mt-3">
        {stages.map((_, i) => (
          <motion.div
            ...
            animate={{
              width: stage === i ? 32 : 8,
              backgroundColor: stage === i ? (stages[stage].color === '#111111' ? '#ff9900' : stages[stage].color) : '#ccc'
            }}
            transition={{ duration: 0.3 }}
      ...
      >
        <motion.div
          ...
          className="w-4 h-4 rounded-full border-2 border-highlightOrange border-t-transparent"
        />
        <span className="text-sm text-gunmetalGray">
          {stage === 0 ? 'Analyzing content...' : `Optimizing for ${stages[stage].platform}...`}
        </span>
      </motion.div>

      {/* Platform logos row */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 pb-4">
        <motion.div
          animate={{
            scale: stage === 1 ? 1.15 : 1,
            opacity: stage === 1 ? 1 : 0.4,
            borderColor: stage === 1 ? '#0077b5' : '#e9eaeb'
          }}
          transition={{ duration: 0.3 }}
          className="w-10 h-10 rounded-medium bg-canvasWhite border-2 border-fadedStone flex items-center justify-center"
        >
        ...
        <motion.div
          animate={{
            ...
            borderColor: stage === 2 ? '#111111' : '#e9eaeb'
          }}
          transition={{ duration: 0.3 }}
          className="w-10 h-10 rounded-medium bg-canvasWhite border-2 border-fadedStone flex items-center justify-center"
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" fill="#111111"/></svg>
        </motion.div>
        <motion.div
          animate={{
            scale: stage === 3 ? 1.15 : 1,
            opacity: stage === 3 ? 1 : 0.4,
            borderColor: stage === 3 ? '#e1306c' : '#e9eaeb'
          }}
          transition={{ duration: 0.3 }}
          className="w-10 h-10 rounded-medium bg-canvasWhite border-2 border-fadedStone flex items-center justify-center"
        >
...
const HistoryDemo = () => {
  ...
  return (
    <div className="bg-offWhiteSage rounded-cards border border-fadedStone p-5 h-[360px] flex flex-col overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between mb-3 flex-shrink-0">
        <div>
          <h3 className="text-midnightInk font-semibold text-base">Your Generated Posts</h3>
          <p className="text-gunmetalGray text-xs">Click to reuse or edit</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1 rounded-navigation bg-highlightOrange/10 border border-highlightOrange/30">
          <span className="text-highlightOrange text-xs font-semibold">{posts.length} posts</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-3 min-h-0">
          ...
          {posts.map((post, i) => (
            <motion.div
              ...
              animate={{
                ...
                backgroundColor: i === selectedPost ? ['#ff990015'] : ['transparent']
              }}
              transition={{ duration: 0.3 }}
              onClick={() => setSelectedPost(i)}
              className={`p-2.5 rounded-small cursor-pointer border transition-all ${
                i === selectedPost ? 'border-highlightOrange/40' : 'border-fadedStone hover:border-softConcrete'
              }`}
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                />
                <div className="text-xs font-medium text-gunmetalGray truncate">{post.platform}</div>
              </div>
              <div className="text-midnightInk text-xs font-medium truncate leading-tight">{post.title}</div>
              <div className="flex justify-between text-[10px] text-gunmetalGray mt-1">
                <span>{post.date}</span>
        ...
        <div className="flex-1 relative min-h-0">
          {posts.map((post, i) => (
            ...
            >
              {/* Platform Header */}
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-fadedStone flex-shrink-0">
                <img
                  src={post.logo}
                  alt={post.platform}
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-midnightInk font-semibold text-sm truncate">{post.platform}</h4>
                  <p className="text-gunmetalGray text-[10px]">Generated from your commit</p>
                </div>
              </div>

              {/* Post Content Preview */}
              <div className="flex-1 bg-canvasWhite rounded-small p-3 mb-3 flex-shrink-0 border border-fadedStone">
                <div className="text-midnightInk text-xs font-medium mb-1">{post.title}</div>
                <div className="text-gunmetalGray text-[11px] leading-relaxed line-clamp-2">{post.preview}</div>
              </div>

              {/* Stats */}
              <div className="flex gap-2 mb-3 flex-shrink-0">
                <div className="flex-1 text-center p-1.5 rounded-small bg-canvasWhite border border-fadedStone">
                  <div className="text-highlightOrange text-xs font-bold">{post.engagement}</div>
                  <div className="text-gunmetalGray text-[9px]">Engagement</div>
                </div>
                <div className="flex-1 text-center p-1.5 rounded-small bg-canvasWhite border border-fadedStone">
                  <div className="text-highlightOrange text-xs font-bold">{post.reach}</div>
                  <div className="text-gunmetalGray text-[9px]">Reach</div>
                </div>
              </div>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="flex-1 py-2 rounded-small bg-actionBlack text-canvasWhite text-xs font-medium flex items-center justify-center gap-1.5"
                >
                  <span>♻️</span> Reuse
                </motion.button>
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="flex-1 py-2 rounded-small bg-canvasWhite border border-fadedStone text-midnightInk text-xs font-medium flex items-center justify-center gap-1.5 hover:bg-offWhiteSage"
                >
                  <span>✏️</span> Edit
                </motion.button>
              </div>
...
const GitHubDemo = () => {
  ...
  return (
    <div className="bg-offWhiteSage rounded-cards border border-fadedStone p-6 h-[360px] flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="text-center mt-4">
        <div className="text-midnightInk font-bold text-sm mb-1">
          {commits === 0 ? 'Listening for commits...' : `${commits} new commit${commits > 1 ? 's' : ''}`}
        </div>
        <div className="flex items-center gap-2 justify-center text-xs text-gunmetalGray">
          <motion.div
  ...
  return (
    <section className="relative z-10 py-24 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-midnightInk mb-4 font-geist">
            Powerful Features
          </h2>
          <p className="text-xl text-gunmetalGray max-w-2xl mx-auto">
                ...
                <div className={isEven ? 'order-1' : 'order-2 lg:order-1'}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-medium bg-offWhiteSage border border-fadedStone flex items-center justify-center`}>
                        <Icon className="w-7 h-7 text-midnightInk" />
                      </div>
                      <h3 className="text-3xl font-bold text-midnightInk">{feature.title}</h3>
                    </div>

                    {/* Description */}
                    <p className="text-xl text-gunmetalGray leading-relaxed">
                      {feature.description}
                    </p>

                    {/* Detail content */}
                    <p className="text-gunmetalGray leading-relaxed">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {feature.stats.map((stat, idx) => (
                        <div
                          key={idx}
                          className="bg-offWhiteSage rounded-small border border-fadedStone px-4 py-2"
                        >
                          <div className="text-sm font-semibold text-midnightInk">
                            {stat}
                          </div>
                        </div>
                      ))}
